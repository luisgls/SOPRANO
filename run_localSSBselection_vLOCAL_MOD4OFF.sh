...
####Version 3.1
## Beatriz Monterde 2025: Modification to run SOPRANO without mutations in the OFF target region

BASEDIR=/data/rds/DMP/UCEC/EVOLIMMU/bmonterde/tools/SOPRANO
SUPA=$BASEDIR/data 
TRANS=$BASEDIR/data/ensemble_transcriptID.fasta
TMP=$BASEDIR/tmp/
FASTA=/data/scratch/DMP/UCEC/EVOLIMMU/SOPRANO/ensembl_downloads/homo_sapiens/110_GRCh38/Homo_sapiens.GRCh38.dna.toplevel.fa
GENOME=/data/scratch/DMP/UCEC/EVOLIMMU/SOPRANO/ensembl_downloads/homo_sapiens/110_GRCh38/Homo_sapiens.GRCh38.dna.toplevel.chrom
...
cut -f1 $BED.tmp | sort -u | fgrep -w -f - $SUPA/ensemble_transcript_protein_min30.length > $TMP/$NAME.protein_length_filt.txt
cut -f1 $BED.tmp | sort -u | fgrep -w -f - $SUPA/ensemble_transcript_min30.length > $TMP/$NAME.transcript_length_filt.txt

if [[ ! -s $TMP/$NAME.protein_length_filt.txt ]] ; then
    echo "File $TMP/$NAME.protein_length_filt.txt is not there or is empty, aborting."
    exit 1
fi

if [[ ! -s $TMP/$NAME.transcript_length_filt.txt ]] ; then
    echo "File $TMP/$NAME.transcript_length_filt.txt is not there or is empty, aborting."
    exit 1
fi
...
if [[ $MODEL = "random" ]];
    then
        ...
        cut -f1,2,3 $BED.tmp > $TMP/$NAME.exclusion.ori
        ## Excluding the first two aminoacids of the transcript to be tested
        cut -f1 $BED.tmp | awk '{OFS="\t"}{print $1,0,2}' | sortBed -i stdin  >> $TMP/$NAME.exclusion.ori
        
        ##Sort excluded regions file
        sortBed -i $TMP/$NAME.exclusion.ori > $TMP/$NAME.exclusion.bed
        bedtools shuffle -i $BED.tmp -g $TMP/$NAME.protein_length_filt.txt -excl $TMP/$NAME.exclusion.bed -chrom > $TMP/$NAME.epitopes.ori2
    ...
    then
        ...
        bedtools shuffle -i $BED.tmp -g $TMP/$NAME.protein_length_filt.txt -incl $TARGET -noOverlapping > $TMP/$NAME.epitopes.ori2
    else
        echo "Target file to randomize regions not provided, using default (all)"
    fi
else
    ##If non randomized
        sort -u $BED.tmp > $TMP/$NAME.epitopes.ori2    
...
if [ -f "$TMP/$NAME.listA" ]; then
	# Check if the file is not empty
	if [ ! -s "$TMP/$NAME.listA" ]; then
                echo "Error: File $TMP/$NAME.listA is empty. Stopping script."
                exit 1
        fi
fi
...
then
    Rscript $BASEDIR/scripts/calculateKaKsEpiCorrected_CI_intron_V3_mod4OFF.R $TMP/$NAME.data_epitopes $TMP/$NAME.epitope_NaNs.txt $TMP/$NAME.nonepitope_NaNs.txt $TMP/$NAME.intronic.rate > $OUT/$NAME.SSB_dNdS.txt
else
    Rscript $BASEDIR/scripts/calculateKaKsEpiCorrected_CI_mod4OFF.R $TMP/$NAME.data_epitopes $TMP/$NAME.epitope_NaNs.txt $TMP/$NAME.nonepitope_NaNs.txt > $OUT/$NAME.SSB_dNdS.txt
fi
